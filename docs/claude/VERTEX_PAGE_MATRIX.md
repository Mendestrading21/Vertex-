...
- `vertex/ui/shell/__init__.py` indique « huit espaces » alors que `PRIMARY_NAV` en contient neuf.
- `vertex/ui/pages/options_intel_page.py` dit qu’Options n’est pas le neuvième espace, mais le shell l’inclut.
- `.interface-design/system.md` décrit « Obsidian Copper » tandis que `tokens.css` décrit « Signal Terminal ».
- `tokens.css` utilise le vert comme marque alors que la direction validée réserve le vert au positif.

Traiter ces divergences dans la phase Fondations.
