...
Les corps de requête Flask sont plafonnés à **2 Mio** par `MAX_CONTENT_LENGTH`. La limite est appliquée avant le traitement d’une route et une requête dépassant cette taille reçoit `413 Request Entity Too Large`. Cette borne protège notamment les imports et synchronisations locales, tout en laissant de la marge pour les petits blobs JSON du desk.
