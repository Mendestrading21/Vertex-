...
> **Status: REFERENCE — comportement historique, design supersédé par V4**
...
- **Baseline historique** : la barre reste à huit espaces et `/options` marque
  Opportunités actif. La cible V4 fait d'Options un espace explicite ; le test
  historique doit être migré avec le shell, jamais contourné.
...
- Le rendu historique est supersédé par Obsidian Prism ; conserver uniquement
  les badges sémantiques `--vx-positive` / `--vx-negative` / dim.
...
- `tests/test_options_routes.py` — 10 tests : routes rendent le contrat
  canonique, comportement baseline du nav, chart_id inconnu ⇒ INCONNU honnête,
...
et tiroir explicatif (contrat carte §5/§34). Couleurs du registre V4 : prism
neutre, vert/rouge/ambre financiers et bleu limité à la comparaison. Série vide
