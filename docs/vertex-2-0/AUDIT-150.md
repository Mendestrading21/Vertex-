...
| 053 | Tri, filtre et recherche annoncent leur état | **RÉUSSI** pour Calendrier et Simulateur | `aria-pressed` sur chaque chip de filtre ; compteur « 12 sur 12 événements ». Ailleurs → lots de page. **Nuance apportée au lot 29** : sur `/calendar?view=options`, neuf chips annonçaient un `aria-pressed` qu'aucun clic ne pouvait changer — `calendar.js` sort de `boot()` avant de les câbler, parce que cette vue ne lit pas `/cal-feed`. Un état annoncé mais impossible à modifier est pire qu'aucun état. Ces chips ne sont plus rendus sur cette vue, et la barre dit ce qu'elle mesure à la place. Mesuré en **cliquant** (`tools/vertex_2_0_boutons_morts.py`), pas en lisant le code. |
