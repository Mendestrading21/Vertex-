...
L’image exacte doit être jointe au ticket GitHub principal de la refonte et fournie à Claude Code au début de chaque session visuelle importante.

Nom recommandé dans le dépôt si elle est ajoutée ultérieurement :

```text
docs/vertex-v4/reference/vertex-v4-master-reference.png
```

Cette image est une **direction de design**, pas une source de chiffres ni de fonctionnalités fictives.
...
Conserver une combinaison proche de :

```text
Interface : Inter
Données / nombres : IBM Plex Mono ou SFMono
```
...
- décision de fusion ;
- transmission de la référence visuelle à Claude Code ;
...
1. Baseline et inventaire des captures.
2. Tokens et identité V4.
3. Shell global.
4. Composants et contrôles.
5. Système de graphiques.
6. Briefing.
7. Marchés.
8. Opportunités.
9. Portefeuille.
...
14. Système.
15. Suivis et mobile.
16. Nettoyage des anciennes couches.
17. QA finale et PR vers `main`.
