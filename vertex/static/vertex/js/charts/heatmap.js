...
function cellColor(v,min,max,scale){
  ...
  /* SÉQUENTIEL (mono-teinte argent) pour une donnée NON signée : IV, surface de vol…
     — une IV n'est ni « bonne » ni « mauvaise », donc pas de vert/rouge. */
  if(scale==='sequential'){
    const t=Math.max(0,Math.min(1,(v-min)/((max-min)||1)));
    return (C.rgba&&C.rgba(C.colors.brand,.06+.44*t))||`rgba(201,205,212,${.06+.44*t})`;}
  /* DIVERGEANT (défaut) : émeraude gains / corail pertes — dérivé du thème. */
