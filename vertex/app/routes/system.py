...
_SW_JS = r"""
const CACHE='td-shell-v244';  // v244 SIGNAL OS · QUATRE ETIQUETTES DE FRAICHEUR ETAIENT DES CONSTANTES. Mon verdict du lot 62 (« Options n'a aucun vocabulaire de fraicheur ») etait FAUX : Options porte bien une etiquette, dans une TROISIEME grammaire que mon instrument ignorait — `.vx-freshness[data-state]`, ni la puce de VX.freshness.chip() ni le badge data-live de freshBadge() — et c'etait exactement la meme constante que celle corrigee la veille. Deuxieme faute de methode, plus grave : ma GRANULARITE etait la page. L'outil du lot 62 rendait « DIT » des qu'UNE etiquette reagissait, donc une page portant un badge honnete ET un badge constant etait declaree saine. Un verdict par page masque un defaut par badge. Le nouvel instrument apparie les etiquettes par leur CHEMIN DOM entre visite nominale et visite vieillie (+2 h), et rend un verdict PAR BADGE. Il porte un temoin POSITIF pris dans le produit — l'etiquette d'Aujourd'hui, mesuree reactive au lot 62 : si l'instrument ne la voit pas bouger il rend AVEUGLE. Ce temoin m'a repris des le premier passage : en mode demonstration les pages court-circuitent l'evaluation, et TOUT paraissait constant. L'outil abaisse donc `demo` sur les DEUX visites — sur la seule visite vieillie, tout aurait paru reagir pour la mauvaise raison. RESULTAT : cinq constantes, et un motif qui designe la cause — les CINQ puces issues de VX.freshness.chip() reagissaient toutes, les QUATRE badges ecrits a la main mentaient tous. Le defaut ne venait pas des pages : ecrire l'etiquette soi-meme etait plus court que d'aller chercher un age. D'ou VX.freshness.domainChip(), qui lit `domains.<nom>.age_s` de /api/live/status pose par le shell sur chaque page et rend l'honnetete plus courte que le mensonge. Analyse et Options (les deux cartes-VERDICT, la ou l'on conclut) le lisent ; Opportunites et Portefeuille partagent desormais l'age de session que leur propre puce utilisait deja. Sur Options, « DELAYED » — anglais dans une interface francaise — disait la meme chose que les primes aient dix secondes ou trois jours. La cinquieme etiquette, sur Systeme, avait un TEXTE honnete (« Systeme operationnel » decrit la connexion) mais portait la CLASSE de la fraicheur : c'est elle qui l'a fait prendre deux fois pour ce qu'elle n'est pas. Elle passe a `vx-badge-status`, la pilule d'etat deja utilisee six fois sur cette meme page. Mesure apres correction : ZERO constante sur les huit espaces. Huit mutations, huit detectees.
