...
def _valid_dte(value):
    """DTE entier positif réellement reporté, sinon None sans maturité de repli."""
    try:
        numeric = float(value)
...
def _swing_project(spot, iv, delta, cost, theta_burn, dte, days=21):
    ...
    try:
        ...
        dte = _valid_dte(dte)
        if spot <= 0 or cost <= 0 or iv <= 0 or dte is None:
            return None, False
        ...
        swing_ok = bool(dte >= 90 and swing_ret >= 25)   # ≥ 90j (sécurité) + ≥ +25 % (objectif 1-4 sem)
...
def _annotate_swing(board, detail):
    ...
    for c in board or []:
        ...
        c['swing_ret'] = sr
        c['swing_ok'] = ok
        c['swing_status'] = ('SWING_DTE_AVAILABLE' if _valid_dte(c.get('dte')) is not None
                             else 'SWING_DTE_UNAVAILABLE')
    return board
