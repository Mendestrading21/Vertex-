...
7. **Étiquettes de fraîcheur : JAMAIS écrites à la main** (mesuré aux lots 62–64). C'est le défaut qui s'est reproduit le plus mécaniquement de toute la série, parce qu'il suit une pente naturelle : **écrire l'étiquette soi-même est plus court que d'aller chercher un âge**. Résultat mesuré badge par badge, en vieillissant les réponses en vol à +2 h : les **5 puces issues de `VX.freshness.chip()` réagissaient toutes** ; les **4 badges écrits à la main mentaient tous** (« Différé » ou « DELAYED » que la donnée ait 10 s ou 3 jours, l'état « Périmé » structurellement inatteignable) — sur Aujourd'hui, Analyse et Options **sur la carte-Verdict, là où la page conclut**. La règle : passer par **`VX.freshness.chip(VX.freshness.assess({ageMs, live}))`**, ou par **`VX.freshness.domainChip('<domaine>')`** qui lit `domains.<nom>.age_s` de `/api/live/status` (posé par le shell sur `window.__vxStatus` de chaque page, chaque domaine avec son propre seuil serveur). **Ne jamais recopier les seuils** — les emprunter à `VX.freshness.THRESH`, sinon deux tables divergent au premier ajustement et l'écran dit « Différé » là où Marchés dit « À actualiser », sur la même donnée. Âge inconnu → `assess` rend « — », l'aveu honnête, jamais une valeur inventée. Trois pièges mesurés : (a) il reste **deux grammaires** servies — `.vx-fresh-chip[data-state]` et `.vx-freshness[data-live]` (où `frozen` = « Périmé ») — n'en connaître qu'une fait rendre « sans vocabulaire » sur une page qui en a bien un ; (b) une pilule qui décrit la **connexion** (« Système opérationnel ») ne doit pas porter la classe `vx-freshness` mais `vx-badge-status` — un mot honnête dans le mauvais vêtement se fait prendre pour un mensonge ; (c) le **mode démonstration court-circuite** l'évaluation (`if(demo){…DÉMO…}`), donc toute mesure de fraîcheur hors conditions réelles doit forcer la branche non-démo, sinon tout paraît constant. Instruments : `tools/mesurer_fraicheur_dite.py` (avant/après par page) et `tools/mesurer_fraicheur_par_badge.py` (**par badge**, apparié au chemin DOM — un verdict par page masque un défaut par badge). Gardiens : `tests/test_signal_os_fraicheur_lot62.py` et `tests/test_signal_os_fraicheur_par_badge_lot63.py`.
8. **Contrat du Chart Shell : ce qui est passé doit être lu** (lot 64). `tools/mesurer_contrat_chart_shell.py` compare les deux côtés — une clé **passée mais jamais lue** signifie qu'une page **croit dire quelque chose** (nommer sa source, sa limite) que le composant jette en silence. Aujourd'hui : **aucune**, et `tests/test_signal_os_contrat_chart_shell_lot64.py` **appelle la mesure** pour que ça le reste. `C.freshnessBadge` a été **retiré** : `opts.freshness` n'était passé par personne, 0 badge peint sur 4 cartes — l'âge vit dans la provenance en pied (`VX.updateIndicator`). Ne pas le rétablir sans appelant : ce serait un second domicile pour une donnée déjà affichée.
