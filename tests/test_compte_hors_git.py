"""Vertex Test 1.0 — LE NUMÉRO DE COMPTE RÉEL ÉTAIT DANS UN DÉPÔT PUBLIC.
...
| fichier | nature |
|---|---|
| un audit visuel de juillet (archive, retiree du depot) | document d'audit (juillet) |
| le registre de decisions 1.0 (archive, retiree du depot) (D-019) | registre de décisions |
| `vertex/data_sources/ibkr_news.py` | docstring de mesure |
| le banc des adaptateurs G5 (supprime avec le code injoignable) | ×4, dont l'assertion d'anonymisation |
| `tests/test_news_ibkr.py` | docstring de mesure |
| `tests/test_reconciliation_pnl.py` | fixture |
| `tests/test_scan_ibkr.py` | docstring de mesure |
