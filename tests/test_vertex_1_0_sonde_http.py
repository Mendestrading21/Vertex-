"""Vertex 1.0 — UNE PANNE N'EST PAS UNE LENTEUR.
...
from tools.vertex_1_0._sonde_http import (BUDGET_INTERACTIF, PLAFOND_DEFAUT,
                                          Reponse, appeler)

OUTILS = Path(__file__).resolve().parents[1] / "tools" / "vertex_1_0"
...
def test_ne_pas_savoir_n_est_pas_savoir_que_NON():
    ...
    from tools.vertex_1_0._sonde_http import sonder_pret
...
def test_une_surface_vide_AVANT_le_premier_scan_n_est_pas_suspecte():
    ...
    from tools.vertex_1_0.mesurer_surfaces_vides import classer
...
def test_le_gardien_NE_signale_PAS_la_forme_partagee():
    ...
    assert not _APPEL_NU.search("from tools.vertex_1_0._sonde_http import appeler")
