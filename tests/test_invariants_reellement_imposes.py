...
`test_scan_state_invariant`, **exclu du scan par conception** : c'est le
...
(`test_catalyst_type::test_engine_version_bumped_prospective`) impose
...
def test_le_plancher_de_version_est_bien_un_plancher_et_non_une_egalite():
    ...
    import tests.test_catalyst_type as g
...
def test_le_balayage_node_couvre_bien_les_pages_servies():
    ...
    bloc inline (`test_js_syntax_sweep`) — un vrai parseur, pas une
    heuristique.
    ...
    les pages servies."""
    import tests.test_js_syntax_sweep as g
    src = open(g.__file__, encoding='utf-8').read()
...
def test_le_gardien_scan_state_exclut_le_domicile_et_scanne_la_production():
    ...
    import tests.test_scan_state_invariant as g
    src = open(g.__file__, encoding='utf-8').read()
