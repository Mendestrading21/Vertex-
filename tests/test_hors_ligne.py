"""Vertex Test 1.0 · G4 — un âge affiché doit pouvoir REDEVENIR faux tout seul.
...
def test_le_classement_par_chiffre_discrimine_les_trois_familles():
    ...
    from tools.mesures.mesurer_hors_ligne import classer_chiffres
...
def test_la_fenetre_d_observation_est_lue_dans_le_produit():
    ...
    from tools.mesures.mesurer_hors_ligne import periode_retick_s
    p = periode_retick_s()
    assert p > 0
...
def test_l_instrument_refuse_de_mesurer_si_la_periode_est_introuvable():
    ...
    import tools.mesures.mesurer_hors_ligne as mod
    vrai = mod.RACINE
