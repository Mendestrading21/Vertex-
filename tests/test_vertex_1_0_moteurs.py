...
from tools.vertex_1_0 import mesurer_moteurs as _mes  # noqa: E402
...
ISOLES_CONNUS = {
    #  Séparation SIGNAL/ALERT/RECOMMENDATION/DECISION/SIMULE/REEL. Excellente
    #  intention, aucun consommateur : importé par trois fichiers de tests et
    #  par rien d'autre. Décision de suppression = #782 + accord humain.
    'performance_ledger',
}
...
def test_performance_ledger_n_est_atteint_que_par_des_tests():
    """La preuve qui justifie son entrée au recensement. Trois fichiers de tests
    l'importent, ce qui suffit à lui donner l'air vivant dans une recherche
    naïve."""
    import subprocess
    r = subprocess.run(
    ...
    hors_tests = [c for c in citations
                  ...
                  and c != './vertex/engines/performance_ledger.py'
                  and c != './tools/vertex_1_0/mesurer_moteurs.py'
                  and not c.startswith('./docs/')]
    assert not hors_tests, (
        'performance_ledger a desormais un consommateur de production : %s — '
        'le retirer du recensement des isoles' % hors_tests)
